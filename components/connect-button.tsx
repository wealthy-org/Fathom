...
export function ConnectButton({
  ...
  const base =
    className ||
    "inline-flex items-center justify-center rounded-full bg-ink px-4 py-2 text-xs font-semibold text-white transition duration-300 hover:scale-105 disabled:cursor-not-allowed disabled:opacity-60";
  ...
  if (signedIn) {
    return (
      <span className="inline-flex flex-wrap items-center justify-end gap-2">
        <span className="rounded-full border border-ink/15 px-3 py-2 font-mono text-xs text-ink sm:px-4 sm:text-sm">
          {shortAddress(address!)}
        </span>
        <button
          type="button"
          onClick={() => void handleDisconnect()}
          className="rounded-full border border-ink/15 px-3 py-2 text-xs text-ink/70 transition hover:border-red-400/60 hover:text-red-600 sm:px-4 sm:text-sm"
        >
          Disconnect
        </button>
      </span>
